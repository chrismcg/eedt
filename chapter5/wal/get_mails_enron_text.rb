...
DIR_PATH = "/Users/waltermcconnell/projects/chrismcg/eedt/chapter5/data/maildir/#{executive}"
EXCLUDED_DIRS = %w(. .. deleted_items all_documents archiving calendar discussion_threads)
SENT_DIRS = %w(sent sent_items)

sent = CSV.open("#{executive}_txt_sent_data_enron.csv", 'w')
sent << %w(date body)
inbox = CSV.open("#{executive}_txt_inbox_data_enron.csv", 'w')
inbox << %w(date body)
